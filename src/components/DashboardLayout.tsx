...
export default function DashboardLayout({ children }: LayoutProps) {
  ...
  const SidebarContent = ({ className }: { className?: string }) => (
    <div
      className={`flex h-full flex-col bg-black/30 backdrop-blur-xl border-none ${className}`}
    >
      {/* Logo Section */}
            ...
            <AvatarImage
              src={
                userProfile?.avatar_base64
                  ? `data:image/jpeg;base64,${userProfile.avatar_base64}`
                  : undefined
              }
              alt="Profile"
            />
    ...
    <div className="min-h-screen relative bg-transparent">
      ...
      <div className="lg:pl-72">
        ...
        <header className="sticky top-4 z-40 flex h-16 shrink-0 items-center gap-x-4 bg-black/30 backdrop-blur-xl px-4 sm:gap-x-6 sm:px-6 lg:px-8 mx-4 rounded-2xl border border-white/5 shadow-lg shadow-black/10">
          ...
          <button
            type="button"
            className={`lg:hidden p-1 hover:bg-transparent transition-colors focus:outline-none ${
              sidebarOpen ? "text-[#3533cd]" : "text-white"
            }`}
          ...
          <div className="flex flex-1 gap-x-4 self-stretch lg:gap-x-6">
            ...
            <div className="flex items-center gap-x-4 lg:gap-x-6">
              ...
              <Badge
                variant={
                  systemHealth.status === "online" ? "default" : "destructive"
                }
