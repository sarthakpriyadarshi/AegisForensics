...
def add_evidence_record(case_name: str, filename: str, file_path: str, file_hash: str, evidence_metadata: str = None):
    db = SessionLocal()
    try:
        case = db.query(Case).filter_by(name=case_name).first()
        if not case:
...
def add_event(case_name: str, description: str, extra: str = None):
    db = SessionLocal()
    try:
        case = db.query(Case).filter_by(name=case_name).first()
        if not case:
